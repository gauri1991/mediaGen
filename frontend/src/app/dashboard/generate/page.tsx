...
function CostBadge({ modelSlug }: { modelSlug: string }) {
  const model = modelRegistry[modelSlug];
  if (!model) return null;
  const cost = model.providers[model.defaultProvider]?.costEstimate;
  if (!cost) return null;
  return <span className="ml-2 text-xs opacity-75">~${cost.toFixed(3)}</span>;
}
          ...
          {activeJobId && activeStatus ? (
            ...
            <div className="flex items-center justify-center h-full min-h-[400px]">
              <div className="text-center space-y-3">
                ...
                <p className="text-muted-foreground text-sm">Your creations will appear here</p>
              </div>
            </div>
          )}
        </main>
